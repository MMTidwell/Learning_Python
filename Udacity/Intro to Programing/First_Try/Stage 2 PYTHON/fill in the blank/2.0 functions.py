...
def play_level(level, level_answers):
    replaced = []
    level_string = level_choice(level_choice)
    index = 0
    print level_string

    while level_string != None:

        level_split = level_string.split()
        print level_split


        for i in level_split:


            for i in blanks:

                user_ans = raw_input("Fill in the blank: ")

                if user_ans == easy_ans[index]:
    ...
    #replaced.append(user_ans)
    #replaced = " ".join(level_string)
    #return replaced    
