...
def quiz(q_type, q_ans):
    ...
    while count < len(q_ans):
        user_answer = raw_input('Fill in the blank \n{0}: \n'.format(count))
        if user_answer in q_ans:
            for listItem in q_type:
                pattern = re.findall(blanks[count] + '(.*)', str(listItem))
                if pattern:
                    q_replace = q_type.index(listItem)
                    q_type.remove(listItem)
                    q_type.insert(q_replace, user_answer)
                else:
                    continue
            print ' '.join(q_type)
            count += 1
